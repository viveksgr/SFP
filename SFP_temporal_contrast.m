...
fprintf('\n')
rsa_P1 = zeros(3,nanat,length(sampleaxis),2); % Complete
hold on
anat_cell = {};
% Subject - index
    ...
    for ii = 1:length(anat_names)
        ...
        for tpoint = 1:length(sampleaxis)
            if min(sum(raw_vox),sum(mod_vox))>1
                ...
                [wt,t_sc] = ARC_multicomputeWeights_tsc([A2_corr(utl_mask) behav_corr(utl_mask) task_run(utl_mask) sess_run(utl_mask) set_run(utl_mask)], M2_anat(utl_mask));
