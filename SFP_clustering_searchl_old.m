...
dirs3 = {fullfile(root ,'\SFP\Clustering\sfp_behav_s01_cluster');
    fullfile(root ,'\SFP\Clustering\sfp_behav_s02_cluster');
    fullfile(root ,'\SFP\Clustering\sfp_behav_s04_cluster\tmp2')};
...
for ss = [1 2 3] % Subject
    ...
    if settings_.sniffupdate
        ...
        [idx,A1] = kmeans(mainmat,settings_.numClusters,'Replicates',1);
    end
    A1_corr = corrcoef(A1');
    ...
    [idx2,A1] = kmeans(behav_ratings,settings_.numClusters,'Replicates',1);
...
save(fullfile(savepath,'ARC_RSA'),'settings_','rsa_P1_')
