...
dirs3 = {fullfile(root,'SFP\Flow_temporal_peakal\SFP1');
        fullfile(root,'SFP\Flow_temporal_peakal\SFP2');
        fullfile(root,'SFP\Flow_temporal_peakal\SFP3')};
...
% neural_RSA  = reshape(cell2mat(neural_RSA ), [3, nanat, wind]);
...
for nn = 1:nanat
    ...
    plot((1:wind)/10,mean(vertcat(neural_RSA{:,nn})))
...
for nn = 1:nanat
    ...
    yyaxis right
    % plot((1:wind)/10,mean(corrmod,1))
    plot((1:wind)/10,neural_RSA{ss,nn})
