...
for ss = [1 2 3] % Subject
    ...
    if settings_.sniffupdate
        ...
        [idx,A1] = kmeans(mainmat,settings_.numClusters,'Distance',dister,'MaxIter',1000,'Replicates',1000);
    end
    A1_corr = corrcoef(A1');
    ...
    [idx2,A1] = kmeans(behav_ratings,settings_.numClusters2,'MaxIter',1000,'Replicates',1000);
    ...
    thresh(ss)= threshold;
    [idx, idx2,ari(:,ss)] = sfp_decoupleClusteringIndices_prob(idx, idx2, A1_corr, A2_corr, 10000, threshold);
    % Nuisance regressors
    ...
    for ii = 1:length(anat_names)
        ...
        for cnt2 = 1:nvox
            ...
            if (size(S_omat_vals_r,1)>1)
                ...
                [wt2,t_sc2] = ARC_multicomputeWeights_tsc([A2_corr(utl_mask) task_run2(utl_mask) sess_run2(utl_mask) set_run2(utl_mask)], M2_anat(utl_mask));
