...
dirs3 = {fullfile(root ,'\SFP\sfp_behav_s01_cluster2');
    fullfile(root ,'\SFP\sfp_behav_s02_cluster2');
    fullfile(root ,'\SFP\sfp_behav_s04_cluster2')};
...
for ss = [1 2 3] % Subject
    ...
    % onsets = load(fullfile(anatdir,sprintf('conditions_sniffupdate_NEMO%02d.mat',s2)),'onsets');
    % onsets = onsets.onsets;
    % [onsetlist,argsort] = sort(vertcat(onsets{:}));
    % idx = SFP_mapMainToSniff(onsetlist,onsets);
    ...
    for ii = 1:length(anat_names)
        ...
        for cnt2 = 1:nvox
            ...
            if (size(S_omat_vals_r,1)>1)
                ...
                % S_I estimation
                utl_mask1 = logical(triu(ones(length(unique(idx))),1)); % All possible odors
                M1_anat = corrcoef(M1');
                M1_anat_vec = M1_anat(utl_mask1);
                A1_corr_vec = A1_corr(utl_mask1);
                [r1,~] = find(isnan(M1_anat_vec));
                M1_anat_vec(r1,:) = [];
                A1_corr_vec(r1,:) = [];
                % rsa_vec_1(cnt2) = ARC_r2t(fastcorr( M1_anat_vec,A1_corr_vec),sum(utl_mask1(:)));
                ...
                M2_anat_vec(r1,:) = [];
                A2_corr_vec(r1,:) = [];
                % rsa_vec_2(cnt2) =  ARC_r2t(fastcorr( M2_anat_vec,A2_corr_vec),sum(utl_mask(:)));
