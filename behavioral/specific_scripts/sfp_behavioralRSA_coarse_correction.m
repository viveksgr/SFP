...
savepath = 'C:\Work\SFP\Final_plots\Behavioral\Trialwise RSA\coarse_v_fine';
hold on
nconds = 4;
...
for ss = 1:length(dirs)
  ...
  [wt,t_sc] = ARC_multicomputeWeights_tsc([behav_corr(utl_mask) unity(utl_mask) pls_corr(utl_mask)  task_run(utl_mask) sess_run(utl_mask) set_run(utl_mask)], A2_corr(utl_mask) );
    ...
    rsa_pvals(ss,3)=  SFP_crosspvalcalc([wt(2) wt2(2)],[t_sc(2) t_sc2(2)],sum(utl_mask(:)));
 ...
 [wt,t_sc] = ARC_multicomputeWeights_tsc([behav_corr(utl_mask) unity(utl_mask) int_corr(utl_mask)  pls_corr(utl_mask)  task_run(utl_mask) sess_run(utl_mask) set_run(utl_mask)], A2_corr(utl_mask) );
     [wt2,t_sc2] = ARC_multicomputeWeights_tsc([behav_corr2(utl_mask) unity(utl_mask) int_corr(utl_mask)  pls_corr(utl_mask)  task_run(utl_mask) sess_run(utl_mask) set_run(utl_mask)], A2_corr(utl_mask) );
    rsa_P1(ss,4,1) = wt(2);
    rsa_P1(ss,4,2) = wt2(2);
    rsa_P1t(ss,4,1) = t_sc(2);
    rsa_P1t(ss,4,2) = t_sc2(2);
    rsa_pvals(ss,4)=  SFP_crosspvalcalc([wt(2) wt2(2)],[t_sc(2) t_sc2(2)],sum(utl_mask(:)));
...
legend({'Fine Perceptual similarity','Coarse Perceptual similarity'})
