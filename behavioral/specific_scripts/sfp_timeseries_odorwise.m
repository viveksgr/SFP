...
hold on
corrmod = zeros(3,wind,nodor);
corrmod_t = zeros(3,wind,nodor );
...
for ss = 1:length(dirs)
    ...
    for pp = 1:nodor
        odorconttrast = group_vec~=pp;
         b1 = corrcoef(behav_ratings_')';
         utl_mask_neq = utl_mask;
         utl_mask_neq(group_vec==pp,group_vec==pp)=false;   
   
        DM_mat = [b1(utl_mask_neq) unity(utl_mask_neq) task_run(utl_mask_neq) sess_run(utl_mask_neq) set_run(utl_mask_neq)];

        % tic
        fprintf('Subject: %02d, odor: %02d',ss, pp)
        fprintf('\n')
        ...
        for jj = 1:wind
            ...
            [wt2,t_sc2] = ARC_multicomputeWeights_tsc(DM_mat, Fless_corr(utl_mask_neq));
...
imagesc(taxis,1:nodor,tempmat(argsort,:))
% yticks(1:nodor)
