...
dirs = {'D:\Work\SFP\sfp_behav_s01';
    'D:\Work\SFP\sfp_behav_s02';
    'D:\Work\SFP\sfp_behav_s03'};
%   color = [0.8500 0.3250 0.0980; 0.9290 0.6940 0.1250; 0.3010 0.7450 0.9330; 0 0.4470 0.7410];
behav = load(fullfile('D:\Work\ARC\ARC','NEMO_perceptual2.mat'));
corrmat_ = true;
svm_trainer = false;
svm_trainer2 = false;
svm_trainer3 = false;
...
for ss = 1:length(dirs)
  ...
  % Fless_mat = vertcat(feat_mat{:});
    ...
    corrmoda(ss) = mean(Fless_corr(M_on))-mean(Fless_corr(M_off));   
...
bar(corrmoda)
hold on
errorbar(1:3,corrmoda, corrmodb,'.')
