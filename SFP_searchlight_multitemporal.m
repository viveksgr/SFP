...
dirs3 = {fullfile(root,'SFP\Flow_multitemporal\SFP1');
        fullfile(root,'SFP\Flow_multitemporal\SFP2');
        fullfile(root,'SFP\Flow_multitemporal\SFP3')};
...
for ss = [1 2 3] % Subject
    ...
    for ii = 1:length(anat_names)
        ...
        rsa_vec = zeros(nvox,length(sampleaxis));
        for tpoint = 1:length(sampleaxis)

            modelmd_ = load(fullfile(anatdir,'Desniff_multitemp',anat_names{ii},sprintf('TR_%02d',tpoint),'TYPEB_FITHRF.mat'),'modelmd');
            ...
            Fmat = Fless_mat_pruned(:,sampleaxis(tpoint));
            ...
            for cnt2 = 1:nvox
                ...
                if (size(S_omat_vals_r,1)>1)
                    ...
                    k = k+1;
                    waitbar(k/(tnvox*length(sampleaxis)), h, sprintf('Processing S:%02d, %.2f %%', ss, (k/(tnvox*length(sampleaxis))*100))) % Update progress


                    temp = corrcoef(M_reg,Fmat_m(utl_mask));
...
% Group Plot
load('C:\Work\SFP\RSA_timecourse\timecourse.mat')
...
for nn = 1:nanat
    ...
    hold on
    yyaxis left
    meansnifft = mean(sniff_trace,1);
    plot((1:wind)/10,mean(sniff_trace,1))
    ylabel('Sniff Trace')

    yyaxis right
    % plot((1:wind)/10,mean(corrmod,1))
    plot(timeaxis,mean(vertcat(neural_RSA{:,nn})))
...
for nn = 1:nanat
    ...
    hold on
    yyaxis left
    plot((1:wind)/10,sniff_trace(ss,:))
    ylabel('Sniff Trace')

    yyaxis right
    % plot((1:wind)/10,mean(corrmod,1))
    plot(timeaxis,squeeze(neural_RSA{ss,nn}))
