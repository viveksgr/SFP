function [avgAccuracy, pValue,featureRMS] = SFP_logisticRegressionCV_ordinal(y, g)
    if ~iscategorical(y)
        y = categorical(y);
    end
    ...
    % Perform 10-fold CV
    nfolds = 10;
    betaw = zeros((numCategories-1)+(size(g,2)),nfolds);
    for k = 1:nfolds
        testIdx = (cvIndices == k);
        trainIdx = ~testIdx;
        ...
        B = fitmnr(X, y_vec,'ModelType','ordinal');
    ...
    for i = 1:numFeatures
        ...
        featureWeights = betaw(2+i, :); % Adjust if intercepts are at a different position
