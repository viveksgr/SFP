...
for ss = [1 2 3] % Subject
    ...
    A2_ = SFP_splitapply_mean(mainmat,group_vec);
    A2_corr = corrcoef(A2');
    ...
    for ii = 1:length(anat_names)
        ...
        [~,t_sc] = ARC_multicomputeWeights_tsc([A2_corr(utl_mask) behav_corr(utl_mask) task_struct(utl_mask) task_run(utl_mask)], M2_anat(utl_mask));
