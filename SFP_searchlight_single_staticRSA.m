...
dirs3 = {fullfile(root,'SFP\regressionmaps\SFP1');
        fullfile(root,'SFP\regressionmaps\SFP2');
        fullfile(root,'SFP\regressionmaps\SFP3')};
...
for ss = [1 2 3] % Subject
    ...
    Fmat_1_m = corrcoef(Fless_mat_pruned');
    ...
    for ii = 1:length(anat_names)
        ...
        for cnt2 = 1:nvox
            ...
            if (size(S_omat_vals_r,1)>1)
                ...
                [~,temp] = SFP_computeWeights_tsc(Fmat_1_m(utl_mask), behav_mat(utl_mask),M_reg);
...
thr = tinv(1 - 0.001, nchoosek(size(Fless_mat,1),2));
...
savefig(fullfile(savepath,'ARC_RSA_sign'))
print(fullfile(savepath,'ARC_RSA_sign'),'-dpng')
