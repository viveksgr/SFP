...
rsa_cell1 = cell(3,nanat);
rsa_cell2 = cell(3,nanat);
...
for ss = [1 2 3] % Subject
    ...
    for ii = 1:length(anat_names)
        ...
        rsa_mat1 = zeros(length(scanaxis),length(timeaxis));
        rsa_mat2 =  zeros(length(scanaxis),length(timeaxis));
        for scanpoint = 1:length(scanaxis)
            for tpoint = 1:length(timeaxis)
                if min(sum(raw_vox),sum(mod_vox))>1
                    ...
                    % modelmd_ = load(fullfile(anatdir,'desniff',anat_names{ii},'TYPEC_FITHRF_GLMDENOISE.mat'),'modelmd','noisepool');
                      modelmd_ = load(fullfile(anatdir,'Desniff_multitemp',anat_names{ii},sprintf('TR_%02d',scanpoint),'TYPEB_FITHRF.mat'),'modelmd');
                    ...
                    A2_corr = pdist(mainmat(:,timeaxis2(tpoint)+1:timeaxis2(tpoint+1)),"euclidean");
                    ...
                    rsa_mat1(scanpoint,tpoint) = wt(2);

                    M2_anat = corrcoef(S_omat_vals_r( mod_vox,:));
                    [wt,t_sc] = ARC_multicomputeWeights_tsc([A2_corr(utl_mask) task_run(utl_mask) sess_run(utl_mask) set_run(utl_mask)], M2_anat(utl_mask));
                    rsa_mat2(scanpoint,tpoint) = wt(2);
                end
            end
        end
        rsa_cell1{ss,ii}=rsa_mat1;
        rsa_cell2{ss,ii}=rsa_mat2;
        rsa_P1(ss,ii,:,1)= mean(rsa_mat1);
        rsa_P1(ss,ii,:,2)= mean(rsa_mat2);
    end
end
